...
def context_data(request):
    ...
    context = {
        ...
        'page_name': '',
        'page_title': '',
        'system_name': 'Online Library Management System',
        'topbar': True,
        'footer': True,
    }
...
def home(request):
    ...
    context['members'] = models.Member.objects.filter(delete_flag=0, status=1).all().count()
    context['books'] = models.Member.objects.filter(delete_flag=0, status=1).all().count()
...
def members(request):
    ...
    context['members'] = models.Member.objects.filter(delete_flag=0).all()
...
def save_member(request):
    ...
    if request.method == 'POST':
        post = request.POST
        if not post['id'] == '':
            member = models.Member.objects.get(id=post['id'])
...
def delete_member(request, pk=None):
    ...
    if pk is None:
        ...
        try:
            models.Member.objects.filter(pk=pk).update(delete_flag=1)
...
def manage_borrow(request, pk=None):
    ...
    if pk is None:
        ...
        context['borrow'] = models.Borrow.objects.get(id=pk)
    context['members'] = models.Member.objects.filter(delete_flag=0, status=1).all()
